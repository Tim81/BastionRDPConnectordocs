...
Antes de ligar, a aplicação verifica o SKU do Bastion e os respetivos indicadores de funcionalidades, e o estado de energia da VM, e indica o que está em falta se uma verificação falhar. Estas verificações são fail-open: uma verificação que não consiga concluir não bloqueia a ligação. Uma sessão do Azure expirada é a exceção: leva-o de novo ao início de sessão.
...
Para iniciar uma VM parada, selecione **Iniciar**. O botão é substituído por um indicador de progresso enquanto a aplicação consulta o Azure para obter o estado atualizado, uma vez a cada 5 segundos durante até 5 minutos. Quando o estado fica verde, uma notificação confirma que a VM está pronta e o botão **Ligar** fica disponível. Enquanto uma VM arranca, **Iniciar** fica oculto e **Ligar** fica desativado, e o estado de arranque é acompanhado por VM, pelo que nunca aparece na errada ao mudar de VM. Se a VM continuar parada ou desalocada durante cerca de 30 segundos, a aplicação comunica um erro com o nome da VM em vez de esperar os 5 minutos completos.
