...
Avant la connexion, l'application vérifie la SKU Bastion et ses indicateurs de fonctionnalités, ainsi que l'état d'alimentation de la VM, et indique ce qui manque si une vérification échoue. Ces vérifications sont fail-open : une vérification qui ne peut pas aboutir ne bloque pas la connexion. Une session Azure expirée fait exception : elle vous ramène à la connexion.
...
Pour démarrer une VM arrêtée, sélectionnez **Démarrer**. Le bouton est remplacé par un indicateur de progression pendant que l'application interroge Azure pour obtenir l'état actualisé, une fois toutes les 5 secondes pendant jusqu'à 5 minutes. Lorsque l'état passe au vert, une notification confirme que la VM est prête et **Connecter** devient disponible. Pendant le démarrage d'une VM, **Démarrer** est masqué et **Se connecter** est désactivé, et l'état de démarrage est suivi par VM, si bien qu'il n'apparaît jamais sur la mauvaise après un changement de VM. Si la VM reste arrêtée ou désallouée pendant environ 30 secondes, l'application signale une erreur nommant la VM au lieu d'attendre les 5 minutes complètes.
