...
Vor dem Verbinden prüft die Anwendung die Bastion-SKU und ihre Feature-Flags sowie den Energiestatus der VM und nennt Ihnen, was fehlt, wenn eine Prüfung fehlschlägt. Diese Prüfungen sind fail-open: Eine Prüfung, die nicht abgeschlossen werden kann, blockiert die Verbindung nicht. Eine abgelaufene Azure-Sitzung ist die Ausnahme: Sie führt Sie erneut zur Anmeldung.
...
Um eine gestoppte VM zu starten, wählen Sie **Start**. Die Schaltfläche wird durch einen Fortschrittsindikator ersetzt, während die Anwendung Azure alle 5 Sekunden für bis zu 5 Minuten nach dem aktualisierten Zustand abfragt. Wird der Status grün, bestätigt eine Toast-Benachrichtigung, dass die VM bereit ist, und **Verbinden** wird verfügbar. Während eine VM startet, ist **Start** ausgeblendet und **Verbinden** deaktiviert, und der Startzustand wird pro VM verfolgt, sodass er beim Wechsel der VM nie bei der falschen erscheint. Bleibt die VM etwa 30 Sekunden lang gestoppt oder freigegeben, meldet die Anwendung einen Fehler mit dem Namen der VM, statt die vollen 5 Minuten abzuwarten.
